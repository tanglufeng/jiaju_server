...
class AdvController extends AdminController
{
     ...
     * 积分规则配置
     */
    public function scoreRule($page = 1, $r = 15){
        $list = M('ScoreRule')->order("createtime desc")->page($page, $r)->select();
        $totalCount = M('ScoreRule')->count();
        foreach($list as $k=>$v){
        ...
        $builder->buttonNew(U('Adv/editScoreRule'))->buttonDelete(U('Adv/delScoreRule'));
        $builder->keyId()->keyText('name','标题')->keyText('tag','规则标识')->keyText('value','规则值')->keyText('rule_time','规则限制时间（0表示没有限制）')->keyText('status','状态');
        $builder->keyDoAction('Adv/editScoreRule?id=###', L('编辑'))->keyDoAction('Adv/delScoreRule?id=###', L('删除'));
        $builder->data($list);
     ...
     * 添加、编辑积分规则
     */
    public function editScoreRule($id = 0){
         
        if($_POST){
            if(!empty($_POST['name']) && !empty($_POST['tag']) && !empty($_POST['value']) && !empty($_POST['rule_time']) && !empty($_POST['status'])){
                $da['name'] = $_POST['name']; 
                $da['tag'] = $_POST['tag']; 
                $da['value'] = $_POST['value']; 
                $da['rule_time'] = $_POST['rule_time']; 
                $da['createtime'] = time(); 
                $da['status'] = $_POST['status'];
                if($id == 0){
                    $add = M('ScoreRule')->add($da);
                    if($add){
                        $this->success('添加成功',U('Adv/scoreRule'));
                    }
                }else{
                     $update = M('ScoreRule')->where(array('id'=>$id))->save($da);
                    if($update){
                        $this->success('修改成功',U('Adv/scoreRule'));
                    }
                }
        ...
        $data = M('ScoreRule')->where(array('id'=>$id))->find();
        $builder = new AdminConfigBuilder();
        if($id == 0 ){
            $title = '新增积分规则';
        }else{
        ...
        $builder->keyId()->keyText('name','标题')->keyText('tag','规则标识')->keyText('rule_time','生效时间限制时长','单位为<天>')->keyText('value', '积分值')->keyStatus();
        $builder->data($data);
        $builder->buttonSubmit(U('editScoreRule'), $id == 0 ? L('新增') : L('编辑'))->buttonBack();
        $builder->display();
    }
     ...
     * 删除积分规则
     */
    public function delScoreRule($id){
        ...
        $del = M('ScoreRule')->where($map)->delete();
        if($del){
            $this->success('删除成功');
        }else{
            $this->error('删除失败');
