...
class ActionLimitController extends AdminController
{
     ...
     * 用户日志--设备报警记录
     */
    public function callPoliceLog($page = 1, $r = 15){
        $list = M('DeviceCallPoliceLog')->order("createtime desc")->page($page, $r)->select();
        $totalCount = M('DeviceCallPoliceLog')->count();
        foreach($list as $k=>$v){
            $list[$k]['e_id'] = get_table_field($v['e_id'],'id','e_name','Equipment');
            $list[$k]['uid'] = get_table_field($v['uid'],'id','username','UcenterMember');
        }
        $builder = new AdminListBuilder();
        $builder->title('设备报警日志');
        $builder->meta_title = L('设备报警日志');
        $builder->buttonDelete(U('Equipment/delcallPoliceLog'));
        $builder->keyId()->keyText('e_id','设备名称')->keyText('uid','设备管理员')->keyText('content','报警内容')->keyTime('createtime','报警时间')->keyStatus('status','状态');
        $builder->keyDoAction('Equipment/delcallPoliceLog?id=###', L('删除'));
        $builder->data($list);
     ...
     * 删除设备报警记录
     */
    public function delcallPoliceLog($id){
        ...
        $del = M('DeviceCallPoliceLog')->where($map)->delete();
        if($del){
            $this->success('删除成功');
        }else{
            $this->error('删除失败');
     ...
     * 用户日志--用户充值记录
     */
    public function userPayScore($page = 1, $r = 15){
        $list = M('ScorePayLog')->order("createtime desc")->page($page, $r)->select();
        $totalCount = M('ScorePayLog')->count();
        foreach($list as $k=>$v){
            $list[$k]['uid'] = get_table_field($v['uid'],'id','username','UcenterMember');
        }
        $builder = new AdminListBuilder();
        $builder->title('用户充值记录');
        $builder->meta_title = L('用户充值记录');
        $builder->buttonDelete(U('Equipment/deluserPayScore'));
        $builder->keyId()->keyText('uid','消费用户')->keyText('score','充值积分')->keyText('money','充值金额')->keyTime('createtime','充值时间');
        $builder->keyDoAction('Equipment/deluserPayScore?id=###', L('删除'));
        $builder->data($list);
        $builder->pagination($totalCount, $r);
        $builder->display();
    }
     ...
     * 删除用户充值记录
     */
    public function deluserPayScore($id){
         $id = array_unique((array)I('id',0));
        ...
        $map = array('id' => array('in', $id) );
        $del = M('ScorePayLog')->where($map)->delete();
        if($del){
            $this->success('删除成功');
        }else{
            $this->error('删除失败');
