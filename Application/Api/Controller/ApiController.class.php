...
abstract class ApiController extends Controller {
    ...
    public function _initialize() {
        ...
        //判断用户在线状态
        $uid=I("uid");
        $res=M('EquipmentAuthRule')->where(array('uid'=>$uid,'status'=>1,'is_online'=>1))->find();
        if($res){
            $data['is_online']=0;
            $data['online_time']=NOW_TIME;
            M('EquipmentAuthRule')->where(array('uid'=>$uid,'status'=>1))->save($data);//取得该设备成员列表
        }else{
            // $nowtime=time();   //获取现在的时间戳
            // $starttime=mktime(0,0,0,date("m"),1,date("H"));
            // dump(($starttime-$nowtime)/3600);
